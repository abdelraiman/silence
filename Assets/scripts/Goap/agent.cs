...
public class GoapAgent : MonoBehaviour
{
    ...
    [SerializeField] Transform meatShopCounter_Survivor; // Survivor goes here to get meat
    [SerializeField] Transform meatShopCounter_Butcher;  // Butcher cooks meat here
    ...
    void Awake()
    {
        ...
        if (CompareTag("Survivor")) role = AgentRole.Survivor;
        if (CompareTag("Woodworker")) role = AgentRole.Woodworker;
        if (CompareTag("Butcher")) role = AgentRole.Butcher;

        GameObject survivorObj = GameObject.FindGameObjectWithTag("Survivor");
        if (survivorObj != null) survivorRef = survivorObj.GetComponent<GoapAgent>();
        else Debug.LogWarning("No Survivor found in the scene!");
    }
    ...
    void SetupBeliefs()
    {
        ...
        if (role == AgentRole.Survivor)
        {
            ...
            factory.AddBelief("NeedsFood", () => meatCount <= 0);
            ...
            factory.AddLocationBelief("AgentAtMeatShop", 5f, meatShopCounter_Survivor);
        }
        ...
        if (role == AgentRole.Woodworker)
        {
            ...
            factory.AddBelief("RequestActiveAtCounter", () => beliefs["SurvivorAtWoodShop"].Evaluate() && beliefs["SurvivorNeedsWood"].Evaluate());
            factory.AddBelief("SurvivorStocked3", () => survivorRef != null && survivorRef.woodCount >= 3);
        }

        // Butcher beliefs
        if (role == AgentRole.Butcher)
        {
            factory.AddBelief("HasCookedMeat", () => meatCount > 0);
            factory.AddLocationBelief("AgentAtMeatShop", 5f, meatShopCounter_Butcher);
        }
    }
    ...
    void SetupActions()
    {
        ...
        if (role == AgentRole.Survivor)
        {
            ...
            actions.Add(new AgentAction.Builder("Eat")
                ...
                .AddPrecondition(beliefs["NeedsFood"])
                .AddEffect(beliefs["HasCookedMeat"])
                .Build());

            actions.Add(new AgentAction.Builder("MoveToMeatShop")
                .WithStrategy(new MoveStrategy(mover, () => meatShopCounter_Survivor.position, movementState, moveSpeed))
                .AddPrecondition(beliefs["NeedsFood"])
                .AddEffect(beliefs["AgentAtMeatShop"])
                .Build());

            actions.Add(new AgentAction.Builder("PickUpMeat")
                .WithStrategy(new RepeatCallbackUntilStrategy(1f,
                    () => meatCount > 0,
                    () => { if (meatCount < 1) meatCount++; }))
                .AddPrecondition(beliefs["AgentAtMeatShop"])
                .AddPrecondition(beliefs["NeedsFood"])
                .AddEffect(beliefs["HasCookedMeat"])
                .Build());
        }
        ...
        if (role == AgentRole.Butcher)
        {
            actions.Add(new AgentAction.Builder("CookMeat")
                .WithStrategy(new RepeatCallbackUntilStrategy(1f,
                    () => meatCount >= 5,
                    () => { meatCount = Mathf.Min(meatCount + 1, 5); }))
                .AddPrecondition(beliefs["AgentAtMeatShop"])
                .AddEffect(beliefs["HasCookedMeat"])
                .Build());
        }
    ...
    void SetupGoals()
    {
        ...
        if (role == AgentRole.Survivor)
        {
            goals.Add(new AgentGoal.Builder("EatFood")
                .WithPriority(4)
                .WithDesiredEffect(beliefs["HasCookedMeat"])
                .Build());
        }

        if (role == AgentRole.Butcher)
        {
            goals.Add(new AgentGoal.Builder("CookMeatGoal")
                .WithPriority(3)
                .WithDesiredEffect(beliefs["HasCookedMeat"])
                .Build());
        }
    ...
    void UpdateFireWood()
    {
        if (fireplaceWood >= 1) fireplaceWood = Mathf.Max(0, fireplaceWood - 1);
    }
    ...
    void Update()
    {
        if (role == AgentRole.Survivor)
        {
            ...
            if (fireplaceWood > 0) firewoodBurnTimer.Tick(Time.deltaTime);
        }

        if (currentAction == null) CalculatePlan();
    ...
    void CalculatePlan()
    {
        ...
        HashSet<AgentGoal> goalsToCheck = currentGoal != null
            ? new HashSet<AgentGoal>(goals.Where(g => g.Priority > priorityLevel))
            : goals;

        var potentialPlan = gPlanner.Plan(this, goalsToCheck, lastGoal);
        if (potentialPlan != null) actionPlan = potentialPlan;

        try { Debug.Log($"[{gameObject.tag}/{role}]{priorityLevel} - {currentGoal.Name}"); }
        catch { }
    }
}


/* old code just incase smt breaks
